    ...
    <div className="min-h-screen pb-24 px-4 pt-5 bg-white dark:bg-black text-black dark:text-white relative overflow-hidden transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <img
          ...
          alt="Profile"
          className={`w-11 h-11 rounded-full object-cover border-2 border-white dark:border-gray-800 transition-transform cursor-pointer ${
            showProfile ? '' : 'hover:scale-105'
          }`}
      ...
      {/* Profile Section */}
      <div
        className={`absolute top-16 left-4 z-30 w-64 transition-all duration-300 ${
          showProfile
            ? 'opacity-100 scale-100'
            : 'opacity-0 scale-95 pointer-events-none'
        } bg-white/90 dark:bg-[#111111] backdrop-blur-lg p-5 rounded-3xl shadow-2xl`}
      >
        <h3 className="text-xl font-bold mb-1">Profile</h3>
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Your user information goes here.
        </p>
      </div>
      ...
      <div className="flex flex-col items-center mt-4">
        <div className="w-full max-w-sm h-[42vh] rounded-3xl overflow-hidden shadow-2xl bg-gray-300 dark:bg-gray-800 mb-4">
          <img
            src={songQueue[currentIndex]?.image || 'https://placehold.co/600x600'}
            alt="Now Playing"
            className="w-full h-full object-cover"
          />
        </div>
        <h2 className="text-2xl font-bold text-center mb-1">
          {songQueue[currentIndex]?.title || 'Song Title'}
        </h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
          {songQueue[currentIndex]?.artist || 'Artist Name'}
        </p>
      </div>
