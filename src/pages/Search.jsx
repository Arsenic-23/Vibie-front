...
export default function Search() {
  ...
  const fetchResults = useCallback(async (searchValue) => {
    ...
    try {
      const res = await axios.get(`${backendUrl}/search/`, { params: { q: searchValue } });
      ...
      <div>
        ...
        <div className="relative max-w-xl mx-auto">
          <input
            type="text"
            value={input}
            onChange={(e) => { setInput(e.target.value); if (!e.target.value) setSearchSubmitted(false); }}
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2">
            <motion.button
              ...
              className={`p-2 rounded-full ${isListening ? 'bg-purple-600 text-white animate-pulse' : 'bg-gray-200 dark:bg-neutral-800 dark:text-gray-300'}`}
              whileTap={{ scale: 0.9 }}
            >
              <Mic size={18} />
            </motion.button>
